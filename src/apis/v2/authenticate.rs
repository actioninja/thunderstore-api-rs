...
use crate::models::v2::authenticate::{RequestBody, ResponseBody};
...
pub async fn experimental_period_auth_period_complete(
    configuration: &Configuration,
    provider: &str,
    data: RequestBody,
) -> Result<ResponseBody, Error<AuthCompleteError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<AuthCompleteError> =
...
pub async fn auth_validate(configuration: &Configuration) -> Result<(), Error<AuthValidateError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<AuthValidateError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
